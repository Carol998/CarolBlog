...
<form id="write_form" name="w_form" role="w_form" class="writedream-form" action="doWritedream?cid=${cont.id}" method="post">
    <div class="writedream-context">
        ...
        <div style="float: left;margin-top: 20px;margin-left: 110px;background-color: #EBEBEB">
            <input type="text" id="txtTitle" name="txt_Title" value="${cont.title}" class="input-file-title" maxlength="100" placeholder="&nbsp;&nbsp;&nbsp;&nbsp;输入文章标题"  style="height: 33px;width: 1080px;background-color:#EBEBEB;border: 0px" >
        </div>
        <!--富文本编辑器-->
...
<script>
    ...
    $("#dreamland-category li").click(function(){//jquery的click事件
        var val = $(this).text();
        $("#fen").html(val);
        $("#hidden_cat").val(val);   //将值赋给input标签，传递给后台
    });

    //创建可视化编辑器的方法，此时为页面加载完成后执行的方法
    //uploadJson:指定上传文件的后台映射URL路径
    //fileManagerJson：指定浏览远程图片的后台映射URL
    $(function () {
